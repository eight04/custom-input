var Emitter = require("event-lite");

function findNearestNode(nodes, start, end) {
	if (!nodes.length) return;

	var [left, right] = matchNodes(nodes, start, end).map(r => r.node);

	if (left == right) {
		return left;
	}

	if (start - left.offset - left.viewValue.length <= right.offset - start) {
		return left;
	}
...
function matchNodes(nodes, start, end = start) {
  ...
  const startNode = nodes.find((node, i) =>
    start === end && node.offset + node.viewValue.length >= start ||
    i + 1 >= nodes.length || 
    nodes[i + 1].offset > start);
  if (startNode) {
    left = {
      node: startNode,
      pos: start - startNode.offset
    };
  }
  
  const endNode = nodes.find(node =>
    node.offset + node.viewValue.length >= end
  );
  if (endNode) {
    right = {
      node: endNode,
      pos: end - endNode.offset
    };
  }
...
class Selection {
	constructor(element, nodes) {
		...
		this.nodes = nodes;
		this.range = {
			node: findNearestNode(this.nodes, 0),
			start: 0,
			end: "end"
		};
	}
	selectNearestNode() {
		...
		this.select({
			node: findNearestNode(this.nodes, range.start, range.end),
			start: 0,
			end: "end"
		});
	}
  ...
  matchEntireNode() {
    ...
    const len = this.range.node.viewValue.length;
    const start = this.range.start === "end" ? len : this.range.start;
    const end = this.range.end === "end" ? len : this.range.end;
		...
		this.el.on("keydown", (e) => {
			...
			if (e.keyCode == 37 || e.keyCode == 9 && e.shiftKey && this.sel.hasPrev()) {
				...
				if (this.sel.atNodeEnd()) {
					...
					this.tryFixingError();
					this.sel.selectNext();
				} else if (this.sel.matchEntireNode()) {
          e.preventDefault();
          this.sel.range.node.unset();
          this.digest(this.sel.range.node, this.sel.range.node.viewValue);
        }
			} else if (e.keyCode == 8) {
				// Backspace
